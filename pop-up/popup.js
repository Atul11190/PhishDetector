...
gettingStarted_button.addEventListener('click', function() {
  ...
  const imageUrl = chrome.runtime.getURL('../images/gettingstarted.svg');
...
function sendEmailToServer(sender, subject, emailBody) {
  ...
  var data = {
    message: JSON.stringify(emailData)  // <-- ye line change kiya, server expects "message" key
  };
...
post_request_button.addEventListener('click', async function() {
  ...
  try {
    ...
    const response = await fetch('https://127.0.0.1:8443/', {
      method: 'POST',
      headers: {
